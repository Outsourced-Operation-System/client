<template>
    <el-table ref="tableRef" :data="bundleList" style="width: 100%" border stripe v-loading="loading"
        ...
        <el-table-column prop="total_value" label="总货值" width="110" align="right">
            <template #default="scope">
                ¥{{ scope.row.total_value?.toFixed(2) || '0.00' }}
            </template>
        </el-table-column>
        <el-table-column prop="main_value" label="主品货值" width="110" align="right">
            <template #default="scope">
                ¥{{ scope.row.main_value?.toFixed(2) || '0.00' }}
            </template>
        </el-table-column>
        <el-table-column prop="gift_value" label="赠品货值" width="110" align="right">
            <template #default="scope">
        ...
        <el-table-column label="操作" min-width="180" fixed="right" align="center">
            <template #default="scope">
                ...
                <el-button link type="primary" size="small" @click="$emit('export', scope.row)">导出</el-button>
            </template>
        </el-table-column>
    </el-table>
</template>
